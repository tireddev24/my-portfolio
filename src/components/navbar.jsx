    ...
    const closeBurger = () => {
        ...
        return (
            ...
            dark:shadow-darkText '>  
                ...
                <div onClick={props.toggleMenu}
                 className ={ `  block wide:hidden scale-75 phoneLg:scale-100 cursor-pointer mt-2 phoneLg:ml-2 -mr-4 ease-linear duration-500` }>
                    <div  className=' *:duration-500 '>
                        <div className={`${burgerClass} relative w-[35px] h-[0.5em] rounded-[0.5em] mb-1 bg-darkpink dark:bg-darkText`} ></div>
                        <div className={`${burgerClass} relative w-[35px] h-[0.5em] rounded-[0.5em] mb-1 bg-darkpink dark:bg-darkText`} ></div>
                        <div className={`${burgerClass} relative w-[35px] h-[0.5em] rounded-[0.5em] mb-1 bg-darkpink dark:bg-darkText`} ></div>
                    </div>
                ...
                {/* // inside burger  */}
                <div className={`${show? 'opacity-100' : 'opacity-0' }  scale-90 phoneLg:scale-100 bg-darkpink dark:bg-[#063248] text-white absolute  rounded-lg  z-10 -top-3 phoneLg:top-0 -left-5 phoneLg:-left-1 duration-700`} onClick={props.toggleMenu}>
                  <div className='move-left fixed scale-90 left-[1px] top-[15px]'>
                    ...
                    { links.map((prop) => 
                    (<>
                        <li  key={prop.key} className={`relative py-1 cursor-pointer  duration-500`} > 
             ...
             <div className='hidden wide:flex  items-center justify-between text-xl uppercase font-semibold tracking-wide rounded-lg '>
                   <ul className='list-none flex justify-between wide_desktop:*:mx-10 *:mx-5  *:border-transparent *:rouned-lg '>
                    { links.map((prop, key) => 
                    (<>

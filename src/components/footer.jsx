...
const linkclass = " tablet:p-2 tablet:h-14  px-1 text-4xl  tablet:w-16 rounded-md  tablet:-mr-1 z-30 duration-500  tablet:text-5xl"
 ...
 function Footer (){
    const links = [
        { link:'Email', data: 'mailto:michaelamao442@gmail.com', icon: <SiGmail className={`${linkclass} text-red-500 ` }  />, color:'before:brightness-105 dark:before:brightness-100 before:bg-gradient-to-tr before:from-red-500 before:from-62% before:via-[#f2a60c] before:via-27% before:to-[#08851b]  *:hover:text-white ' },
        ...
        { link:'Github', data: 'https://github.com/mich-demo23', icon: <PiGithubLogoBold className={`${linkclass} text-[#2e4053]`} />, color: 'before:bg-gray-600 brightness-150 dark:before:bg-[#2e4053] *:hover:text-white' },
        ...
        <footer  className=' appear py-2 pt-5 text-center duration-100 ease-linear border-t-2 border-[#8a045c] dark:border-darkText border-solid' id='contact'>
            ...
            <div className='my-3  mb-1 *:tracking-wider flex items-center *:max-w-max gap-4 tablet:gap-2 justify-center tablet:items-start laptop:flex laptop:items-center laptop:justify-center tablet:grid tablet:grid-cols-2  laptop:gap-20'>
            {
                ...
                (   
                        ...
                        {prop.icon}
                        <p className={` mr-2 text-xl z-50 duration-300 hidden tablet:block font-semibold`}>{prop.link}</p>
                        </a>                     
